...
bool isFinished[100];
int sequence[100];
int S,f,indx,arr[100];
void isSafe(int N,int M)
{
    int i,j,work[100],count=0,s=0;
    for(i=0;i<M;i++)
        work[i]=available[i];
    for(i=0;i<100;i++)
        isFinished[i]=false;
...
int main()
{
    ...
    for(i=0;i<N;i++)
        ...
        printf("Enter process index to request:\n");

        int indx,arr[100];
        printf("Enter the process number for resource request :");
        scanf("%d",&indx);
       
        printf("Enter the safesequence count :");
        scanf("%d",&S);
        printf("Enter the requested instances for type :");
        for(i=0;i<M;i++)
            scanf("%d",&arr[i]);
